...
const fs = require('fs');
const content = fs.readFileSync('components/PricingPage.tsx', 'utf8');
        ...
        if (char === '{' || char === '(' || char === '[') {
            ...
            if (stack.length === 0) {
                ...
                if ((char === '}' && last.char !== '{') ||
                    (char === ')' && last.char !== '(') ||
                    (char === ']' && last.char !== '[')) {
                    console.log(`Mismatched ${char} at line ${i + 1}, col ${j + 1}. Expected match for ${last.char} from line ${last.line}`);
                }
            }
        }
    }
