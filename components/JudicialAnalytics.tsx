...
const ReportSummary = ({ report, onClose }: { report: any, onClose: () => void }) => {
  ...
  return (
    <motion.div 
      ...
      exit={{ opacity: 0, y: 20 }}
      className="bg-white border-2 border-primary/20 rounded-[2.5rem] p-12 shadow-2xl relative overflow-hidden"
    >
      <div className="absolute top-0 right-0 p-8">
        <button onClick={onClose} className="p-3 bg-slate-50 rounded-xl hover:bg-slate-100 transition-all">
      ...
      <div className="flex items-center gap-6 mb-12">
        ...
        <div>
          <h2 className="text-3xl font-bold text-slate-900 tracking-tight">Intelligence Playbook</h2>
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mt-1">Generated: {new Date(report.generated_at).toLocaleDateString()}</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        <div className="space-y-10">
          <section className="p-8 bg-slate-50/50 rounded-[2rem] border border-slate-100">
            <h4 className="flex items-center gap-3 text-sm font-black uppercase tracking-widest text-slate-900 mb-4">
              <Zap className="w-4 h-4 text-amber-500" /> Executive Summary
            </h4>
            <p className="text-base text-slate-700 leading-relaxed font-medium">{s.executive_summary}</p>
          </section>

          <section className="p-8 bg-emerald-50/30 rounded-[2rem] border border-emerald-100/50">
            <h4 className="flex items-center gap-3 text-sm font-black uppercase tracking-widest text-emerald-600 mb-4">
              <TrendingUp className="w-4 h-4" /> Litigation Tactics
            </h4>
            <ul className="space-y-4">
              {s.litigation_tactics.map((t: string, i: number) => (
        ...
        <div className="space-y-10">
          <section className="p-8 bg-primary/5 rounded-[2rem] border border-primary/10">
            <h4 className="flex items-center gap-3 text-sm font-black uppercase tracking-widest text-primary mb-4">
              <AlertCircle className="w-4 h-4" /> Hostile Territory
            </h4>
            <p className="text-sm text-slate-600 font-medium leading-relaxed">{s.hostile_territory}</p>
            <div className="mt-6 flex items-center gap-2 px-4 py-2 bg-white rounded-xl border border-primary/10 w-fit">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Risk Level:</span>
              <span className={`text-[10px] font-black uppercase tracking-widest ${s.risk_level === 'low' ? 'text-emerald-500' : 'text-primary'}`}>{s.risk_level}</span>
            </div>
          </section>

          <section className="p-8 bg-slate-50 rounded-[2rem] border border-slate-100">
            <h4 className="flex items-center gap-3 text-sm font-black uppercase tracking-widest text-slate-900 mb-4">
              <BookOpen className="w-4 h-4 text-blue-500" /> Citation Strategy
            </h4>
            <p className="text-sm text-slate-600 font-medium leading-relaxed">{s.citation_strategy}</p>
          </section>
        </div>
      </div>

      <div className="mt-12 pt-10 border-t border-slate-100 flex justify-between items-center">
        <p className="text-[10px] text-slate-400 font-bold italic">This report is for internal strategic use. Verify citations before filing.</p>
        <button className="px-8 py-3 bg-slate-900 text-white rounded-xl text-[11px] font-bold uppercase tracking-widest shadow-xl flex items-center gap-2">
...
const JudicialAnalytics: React.FC<JudicialAnalyticsProps> = ({ activeSubView = 'Judge directory' }) => {
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const res = await apiClient.post('/api/analytics/reports/generate', { judgeId: selectedJudge.id });
  ...
  const handleChatSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!chatInput.trim() || !selectedJudge) return;
    const userMsg = chatInput;
    setChatMessages(prev => [...prev, { role: 'user', content: userMsg }]);
    setChatInput('');
    
    try {
      const res = await apiClient.post('/api/analytics/chat', { 
      ...
      setChatMessages(prev => [...prev, { role: 'assistant', content: "I'm sorry, I'm having trouble analyzing that pattern right now." }]);
    }
  };
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center bg-slate-50 p-1.5 rounded-2xl border border-slate-100">
                <button 
                  ...
                  className="px-5 py-2.5 bg-white shadow-sm rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-900 border border-slate-100 hover:shadow-md transition-all flex items-center gap-2 group"
                >
                  <div className={`w-1.5 h-1.5 rounded-full ${isGeneratingReport ? 'bg-amber-500 animate-ping' : 'bg-primary animate-pulse'}`}></div>
                  {isGeneratingReport ? 'Synthesizing...' : 'Generate Intelligence Report'}
                </button>
              </div>
            ...
            <ReAreaChart data={[
             ...
             <div className="xl:col-span-9 space-y-10">
                ...
                <div className="min-h-[500px]">
                   <AnimatePresence mode="wait">
                      {reportData ? (
                        <ReportSummary report={reportData} onClose={() => setReportData(null)} />
                      ) : (
                        <motion.div
                          key={activeTab}
                          initial={{ opacity: 0, y: 10 }}
