...
const TESTIMONIALS = [
  ...
  {
    id: 6,
    quote: "The student discount makes Lawlify AI accessible for my research at JKUAT. It's the perfect companion for anyone serious about mastering Kenyan law.",
    author: "Kevin Ochieng",
    role: "Law Student, JKUAT School of Law",
...
const AuthPage: React.FC<AuthPageProps> = ({ onLogin }) => {
  ...
  useEffect(() => {
    ...
    if (urlError) {
      setError(urlError === 'bad_oauth_state' 
        ? 'OAuth state mismatch. Please ensure http://localhost:3000/auth/callback is added to your Supabase "Additional Redirect URLs" settings.' 
        : (urlDesc || urlError));
    }
          ...
          <div className="absolute -bottom-10 left-0 flex gap-3">
            {TESTIMONIALS.map((_, index) => (
              <button
                ...
                className={`h-2 rounded-full transition-all duration-500 ${index === currentTestimonial ? 'w-10 bg-[#FF4444]' : 'w-2 bg-white/20 hover:bg-white/40'
                  }`}
              />
            ))}
          </div>
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <div className="space-y-4">
            ...
            <button
              ...
              disabled={loading}
              className="w-full flex items-center justify-center gap-3 px-4 py-3.5 bg-white border border-gray-200 rounded-xl font-bold text-gray-700 hover:bg-gray-50 hover:border-gray-300 transition-all active:scale-[0.99] disabled:opacity-70 disabled:cursor-not-allowed"
            >
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 px-4 py-3.5 bg-gray-800 text-white rounded-xl font-bold hover:bg-gray-900 transition-all active:scale-[0.99] shadow-lg shadow-black/10"
          >
