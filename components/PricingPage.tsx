...
import { Check, HelpCircle, Building2, GraduationCap, Heart, School, ArrowLeft, Minus, X, Shield, Globe, Zap, Lock, UserCheck, Server, HeadphonesIcon, ArrowRight } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-ai-studio text-white font-sans selection:bg-primary/30 overflow-x-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 pb-24 relative z-10">
        ...
        {/* ── Enterprise-Ready Features Section ── */}
        <motion.div
        ...
        >
          {/* Glow */}
          <div className="absolute inset-0 bg-primary/5 rounded-[3rem] blur-[60px] pointer-events-none" />

          <div className="relative rounded-[3rem] border border-white/10 bg-[#0a0a0a]/80 backdrop-blur-xl overflow-hidden">
            {/* Top accent */}
            <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-primary/40 to-transparent" />

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
              {/* Left: Feature Checklist */}
              <div className="p-12 lg:p-14 border-b lg:border-b-0 lg:border-r border-white/5">
                <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-primary/10 border border-primary/20 rounded-full text-primary text-[10px] font-black uppercase tracking-widest mb-8">
                  <Shield className="w-3 h-3" />
                  Enterprise-Ready
                </div>
                <h2 className="text-3xl md:text-4xl font-black tracking-tight text-white mb-4 leading-tight">
                  Built for the most<br />demanding legal teams
                </h2>
                <p className="text-gray-400 font-medium mb-10 leading-relaxed">
                  Lawlify Enterprise gives your firm the control, compliance, and customisation it needs — without compromise.
                </p>

                <div className="space-y-4">
                  {[
                    { icon: Lock, label: 'Single Sign-On (SSO) & SAML 2.0', color: 'text-primary' },
                    { icon: Server, label: 'On-Premise & Private Cloud Deployments', color: 'text-blue-400' },
                    { icon: Shield, label: 'Advanced Compliance & Audit Logging', color: 'text-green-400' },
                    { icon: UserCheck, label: 'Guaranteed SLA & Priority Response Times', color: 'text-yellow-400' },
                    { icon: Zap, label: 'Custom AI Model Fine-Tuning for Your Firm', color: 'text-purple-400' },
                    { icon: Globe, label: 'Role-Based Access Control (RBAC)', color: 'text-cyan-400' },
                    { icon: HeadphonesIcon, label: 'Dedicated Legal AI Success Manager', color: 'text-orange-400' },
                    ...
                    >
                      <div className={`w-8 h-8 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center shrink-0 group-hover:border-primary/30 transition-all duration-300`}>
                        <Icon className={`w-4 h-4 ${color}`} />
                      </div>
                      <span className="text-sm font-semibold text-gray-300 group-hover:text-white transition-colors duration-200">
                        {label}
                      </span>
                      <Check className="w-4 h-4 text-primary/60 ml-auto shrink-0" />
                    </motion.div>
                  ))}
                </div>
              </div>
              ...
              {/* Right: CTA Panel */}
              <div className="p-12 lg:p-14 flex flex-col justify-center relative overflow-hidden">
                ...
                <div className="relative z-10">
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                      <p className="text-[10px] font-black uppercase tracking-widest text-primary">Enterprise Plan</p>
                      <p className="text-white font-bold text-sm">Custom Pricing Available</p>
                    </div>
                  </div>

                  <h3 className="text-2xl md:text-3xl font-black text-white mb-4 leading-tight">
                    See Lawlify in action<br />for your firm
                  </h3>
                  <p className="text-gray-400 font-medium mb-8 leading-relaxed text-sm">
                    Book a private 45-minute session with our legal AI specialists. We'll map Lawlify directly to your firm's workflow, compliance requirements, and team structure.
                  </p>

                  <div className="space-y-3 mb-10">
                    {[
                      'Live demo with your actual use cases',
                      'Custom pricing & contract negotiation',
                      'Compliance & data residency review',
                    ].map((item, i) => (
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.97 }}
                    onClick={() => navigate('/book-enterprise-demo')}
                    className="w-full flex items-center justify-center gap-3 py-5 bg-primary text-white font-black rounded-2xl hover:bg-primary/90 transition-all shadow-xl shadow-primary/20 text-lg"
                  >
                    Schedule an Enterprise Demo
                    <ArrowRight className="w-5 h-5" />
                  </motion.button>

                  <p className="text-center text-[11px] text-gray-600 font-medium mt-4">
                    No obligation · Confirmation within 24 hours
                  </p>
                </div>
              </div>
            </div>
        ...
        >
          ...
          <button 
            ...
            className="px-8 py-4 bg-white text-black border border-white rounded-2xl font-bold hover:bg-gray-200 transition-colors whitespace-nowrap shadow-lg shadow-white/10"
          >
            Book a Demo
          </button>
        </motion.div>
      ...
      <div className="py-24 px-6 max-w-5xl mx-auto relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-bold tracking-tighter mb-4">Compare Plans</h2>
          <p className="text-gray-400 font-medium">Detailed feature breakdown for every tier</p>
        </div>

        <div className="overflow-x-auto pb-8">
          <div className="min-w-[800px] border border-white/10 rounded-3xl bg-[#0a0a0a]/80 backdrop-blur-xl overflow-hidden">
            {/* Header */}
            <div className="grid grid-cols-4 border-b border-white/10 bg-white/5">
              <div className="p-6 font-bold text-gray-400">Features</div>
              <div className="p-6 font-bold text-center text-white">Free</div>
              <div className="p-6 font-bold text-center text-primary bg-primary/5">Personal</div>
              <div className="p-6 font-bold text-center text-white">Teams</div>
            </div>

            {/* Rows */}
            {[
              { name: 'AI Responses', free: '8/month', personal: 'Unlimited', teams: 'Unlimited' },
              { name: 'Document Vault', free: '500 MB', personal: '5 GB', teams: '50 GB / seat' },
              { name: 'Matter Workspaces', free: '2 Active', personal: 'Unlimited', teams: 'Unlimited' },
              { name: 'AI Associates', free: 'Basic access', personal: 'Up to 10', teams: 'Unlimited' },
              { name: 'Integrations', free: 'Calendar & Draft', personal: '+ Email', teams: 'Custom workflows' },
              { name: 'SSO & Audit Logs', free: false, personal: false, teams: true },
              { name: 'Deployment Options', free: 'Cloud', personal: 'Cloud', teams: 'On-premise / Custom' },
              { name: 'Support', free: 'Community', personal: 'Priority Email', teams: 'Dedicated Success' }
            ].map((row, i) => (
              <div key={i} className="grid grid-cols-4 border-b border-white/5 last:border-0 hover:bg-white/5 transition-colors">
                <div className="p-6 text-sm font-medium text-gray-300 flex items-center">{row.name}</div>
                
                {/* Free Column */}
                <div className="p-6 text-sm flex items-center justify-center text-gray-400">
                  {typeof row.free === 'boolean' ? (
                    row.free ? <Check className="w-5 h-5 text-gray-400" /> : <Minus className="w-5 h-5 text-gray-600" />
                  ) : row.free}
                </div>

                {/* Personal Column */}
                <div className="p-6 text-sm flex items-center justify-center font-bold text-white bg-primary/5">
                  {typeof row.personal === 'boolean' ? (
                    row.personal ? <Check className="w-5 h-5 text-primary" /> : <Minus className="w-5 h-5 text-primary/30" />
                  ) : row.personal}
                </div>

                {/* Teams Column */}
                <div className="p-6 text-sm flex items-center justify-center text-gray-300">
                  {typeof row.teams === 'boolean' ? (
                    row.teams ? <Check className="w-5 h-5 text-gray-300" /> : <Minus className="w-5 h-5 text-gray-600" />
                  ) : row.teams}
                </div>
              </div>
            ))}
