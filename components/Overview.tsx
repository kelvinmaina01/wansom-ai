...
const Overview: React.FC = () => {
  ...
  React.useEffect(() => {
    supabase.auth.getUser().then(({ data: { user } }) => {
      if (user) {
        setUserName(user.user_metadata?.full_name?.split(' ')[0] || user.email?.split('@')[0] || 'Counsel');
      }
    });
  }, []);
    ...
    <div className="flex-1 overflow-y-auto bg-white bg-dots p-8 no-scrollbar">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-left mb-12">
          <h1 className="text-6xl font-bold text-black mb-4 tracking-tighter">Welcome back, <span className="text-primary">{userName}</span></h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
          ...
          <div className="relative overflow-hidden p-8 rounded-[2.5rem] shadow-xl bg-orange-500/90 backdrop-blur-xl text-white group hover:scale-[1.02] transition-transform duration-300 border border-white/10">
            ...
            <div className="relative z-10">
              ...
              <button 
                onClick={() => triggerPrompt("I have a quick legal question. I don't need a full project right now, just focused AI guidance on a specific statutory or procedural inquiry.")}
                className="flex items-center gap-2 px-6 py-3 bg-white/90 backdrop-blur-sm text-orange-600 rounded-xl font-bold hover:bg-white transition-colors shadow-lg shadow-black/10"
              >
                Open Q&A
                <ArrowRight className="w-4 h-4" />
              </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
          <div className="lg:col-span-2 bg-white p-8 rounded-3xl border border-gray-100 shadow-xl shadow-black/5">
            <div 
                ...
                onClick={() => triggerPrompt("Please provide a detailed 'AI Usage Overview' and 'Firm Metrics' report. I want to see how Lawlify is impacting our productivity across different tasks.")}
              >
              <h3 className="text-xl font-semibold text-black group-hover:text-primary transition-colors">AI Usage Overview</h3>
          ...
          <div 
            className="bg-black p-8 rounded-3xl shadow-xl shadow-black/20 text-white cursor-pointer group"
            onClick={() => triggerPrompt("Analyze my 'Quick Stats' and 'Firm Metrics'. Provide a professional summary of my total queries, active projects, and estimated time saved.")}
          >
            <h3 className="text-xl font-semibold mb-8 group-hover:text-primary transition-colors">Quick Stats</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-xl shadow-black/5">
            <div className="flex items-center justify-between mb-8">
              ...
              <button 
                onClick={() => triggerPrompt("Show me a detailed log of my 'Recent Activity'. I want to review my previous actions, from drafted contracts to analyzed statutes.")}
            ...
            <div className="relative pl-4 border-l-2 border-gray-100 space-y-8">
              {MOCK_ACTIVITIES.map((activity) => (
                  <div 
                    ...
                    onClick={() => triggerPrompt(`Please give me a professional summary of this activity: "${activity.user} ${activity.action} ${activity.target}" on ${activity.timestamp.toLocaleString()}.`)}
                  >
                  <div className={`absolute -left-[21px] top-1 w-3 h-3 rounded-full border-2 border-white shadow-sm transition-transform group-hover:scale-125 ${
